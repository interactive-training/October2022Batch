...
public class CRUD_Event {
    ...
    public void user_enters_the_details_for_all_the_mandatory_fields(String EventTitle, String EventType, String EventStartDate, String EventEndDate, String EventTimeDuration, String EventLocation, String EventContent) throws InterruptedException {
        ...
        driver.findElement(By.xpath("/html/body")).sendKeys(EventContent);// iframe.sendKeys(EventContent);
        //driver.findElement(By.xpath("/html/body")).sendKeys("Kanuma follows Sankranti festival.");// iframe.sendKeys(EventContent);
...
//        Assert.assertNull(date, "Date is present."); why the below error
//        java.lang.AssertionError: Date is present.
//                Expected :null
//        Actual   :[[ChromeDriver: chrome on mac (3710e27aeb6240ce85ceadbe6ec9784b)] -> xpath: //tr[4]/td[2]/input]
    }
    ...
    // --------------------------------Create Event's Front-end Verification-------------------------------------------------------------
    ...
    public void UserClicksOnEventsAndChoosesTheEventsOption() throws InterruptedException {
        Actions action = new Actions(driver);// Initiating the Actions class for mouse hover(drop-down)
        ...
        Thread.sleep(3000);
        WebElement events = driver.findElement(By.linkText("Events"));  // By.xpath("(//a[@href='events_web.php'])[1]")      Finding the Events
    ...
    public void userClicksOnTheMoreInfo() {
        Actions action = new Actions(driver);// Initiating the Actions class for mouse hover(drop-down)
    ...
    public void user_should_be_able_to_see_the_event(String EventTitle) {
    ...
    // ------------------------------------------Edit Event-----------------------------------------
    @When("user selects the Edit option for serial number {int} for {string} with {string}")
    public void userSelectsTheEditOptionForSerialNumberSerialNumberForWith(int SerialNumber, String EditField, String EditInfo) {
...
//            }
            ...
            System.out.println("Please give a valid serial number");
        }
    }
    ...
    public void userClicksOnTheMoreInfoOfSerialNumber(int SerialNumber) {
        SerialNumber--;
        Actions action = new Actions(driver);// Initiating the Actions class for mouse hover(drop-down)
        //WebElement moreInfo = driver.findElement(By.xpath("(//div[@class='events_main'])[" + SerialNumber + "]/p[5]/a"));
    ...
    public void user_selects_the_delete_option_for_then_it_should_be_deleted(String EventTitle) {
        ...
        int n = 0;
        if (n <= newTitleElements.size()) {
            String title = newTitleElements.get(n).getText();
            Assert.assertFalse(title.equalsIgnoreCase(EventTitle));
             n++;
        } else if(n > newTitleElements.size()){
...
//Assert.assertFalse(newTitleElements.contains(EventTitle));

//            for (int n = 0; n <= newTitleElements.size(); n++) {
//                // Looping each Event Title
//                String title = newTitleElements.get(n).getText();
////                while (title.equalsIgnoreCase(EventTitle)) {
